...
### 真实数据里的三个形状（量过全部 141 条，不是猜的）

> 2026-08-29 复核：`num=200` 实际回 141 条（feed 自报 `total: 145`）。
> 初稿按 100 条的样本写，下面的数字已按 141 条重量过。

1. **11/141 条 `more` 是空数组，真正的更新说明跑到了 `detail.title` 里。**
   ...
   > 守的就是这条。
2. **`detail` 里的 key 顺序不固定，key 集合也不固定** —— 141 条里 `more` 和
   `title` 条条都有，`stable` 有 97 条，`feature_tips` 有 **44 条**（初稿写「3 条」，
   少算了十四倍）。任何「`more` 是第一个 key」的假设都会漏。
   `feature_tips` 在这 44 条里**全是字符串**（`"mac版可以xxx啦"`，厂商占位文案，
   从来不是更新说明），所以第一条 item pattern 的标点锚定天然把它排除掉——
   它的值前面是 `:`。**但这是数据形状兜住的，不是 pattern 兜住的**：哪天百度把它
   改成数组，它的元素就会开始当 note 渲染。
