...
**App Store updates for apps sold in another country install again.** An app you own that is listed in a storefront other than your account's cannot be updated from its own page — the store answers "App Not Available" — so Duo Updater drives the App Store's Updates list instead and presses the row belonging to that app. Finding the row means reading the app names off the list, and on current macOS that list is drawn as a web page, where a name is not where an ordinary Mac control keeps it. Duo Updater was looking in the ordinary place, found only the App Store sidebar's own headings, and matched nothing — so every one of those updates ended in "the App Store hasn't listed this update yet" while the row sat on screen in plain view. It reads the names where the page actually puts them now. The list is also two columns wide, and a row was previously identified by the horizontal band it sits in — which holds two apps, not one, so the left column answered for both. Each row is now tied to its own button. And if the App Store had been left on some app's page, that page covered the Updates list and the refresh only reloaded the page on top; the list is uncovered first.

**A large App Store update is no longer called a failure for taking its time.** After you tap Relaunch, the App Store swaps the new version in, and Duo Updater waited a fixed 90 seconds for that to happen — a number taken from one small app. Word and Excel are around 2.7 GB each and need roughly twice that, so a perfectly successful update finished under a red "Timed out waiting for the App Store" that stood until the next scheduled check. The wait is now judged on whether the swap is still moving rather than on a stopwatch, and where the App Store gives nothing to watch it waits far longer before giving up. Separately, the App Store sometimes leaves the "Close This App to Update" panel on screen for minutes after the update it belonged to has already landed; that leftover is no longer mistaken for the next update's, which used to send you to relaunch an app that had nothing waiting for it.
