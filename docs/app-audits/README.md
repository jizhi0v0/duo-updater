...
- [x] [**搜狗输入法 (SogouInput)**](com-sogou-inputmethod-sogou.md) · `com.sogou.inputmethod.sogou` — P · 版本读**厂商自己的条件更新接口**（pin `v=0.0.0.1` 装成很旧的客户端去问；已验证不做分段升级，六个历史版本都答同一个最新版）· 返回的版本与 `CFBundleShortVersionString` 四段完全一致，无需任何裁剪 · 哨兵 `1.0.0.1` 由 `update_pack_url` 守卫挡掉 · 不发设备 hash · notes 走更新日志页 · **一键**：自更新包是裸 `Contents<版本>` 双层 zip（无 `.app`），由 `ContentsPayload` 装配后走 Contents 轮换；三个脚本都不跑（`pre.sh` 只在不可写时动、`post.sh` 是 2019 年那版的死代码、`switch.sh 1` 会删词库）· 2026-08-28 / 2026-09-20
- [x] [**豆包输入法 (DoubaoIme)**](com-bytedance-inputmethod-doubaoime.md) · `com.bytedance.inputmethod.doubaoime` — P+C · 比厂商 version code（装机侧在自定义键 `Wave Build Version Number`，CFBundleVersion 是废号 1）· changelog 走 app 自己的更新接口 · 一键 ✓（2026-08-28，走输入法 Contents 轮换而非整包覆盖）· channel-verify ✓ · 2026-08-21
- [x] [**千问输入法 (QianwenIME)**](com-qianwen-inputmethod-desktopime.md) · `com.qianwen.inputmethod.desktopime` — P · app 自己的更新检查是 Quark PUDS protobuf + WSG 加密，读不了，改读官网下载按钮读的那条 JSON · `ch=pcqwenime@homepage_official` 是关键参数，不认识的值静默回落到一个旧包 · 比包名里的四段号（= `CFBundleVersion`），显示前三段 · 官网 `api/download-config` 是坑（自相矛盾 + 官网自己不用）· 无 changelog 页 · **一键 ✓**（dmg → 安装器壳 → `nestedArchivePath` 取内层 zip，装法走输入法 Contents 轮换——厂商自更新 `update.sh` 正是 `renameatx_np` 换 `Contents`，只读安装器那份脚本会判反）· channel-verify + 真包闸链 + **真机红→绿 ✓**（外层 inode、输入源注册、用户数据全部保持；造红用厂商自己的 update.sh）· 2026-09-18
...
- [x] [**Qoder CN IDE**](com-aliyun-lingma-ide.md) · `com.aliyun.lingma.ide` — P C (one-click arm64 zip, `versionTemplate`) · 国内版 IDE，与 Qoder IDE 同源同 commit，但 Team `9DFNGU9AK5`、更新服务器、说明页都分开 · 走 `/api/qodercn/update/…/latest`（抓包得到；VS Code 原路径是 Lingma 的旧表）· 按 `machineId` 灰度，读 app 自己 `storage.json` 里的 id（同国际版）· 回答里的 `lastest` zip 在 CDN 边缘滞后一版，故按版本拼地址 · 2026-09-22
...
- [x] [**Memoh Desktop**](ai-memoh-desktop.md) · `ai.memoh.desktop` — P (one-click arm64 zip + feed sha512) · 真包 2026.9.16-1 解包验证 ✓（Team P9R669C27U，notarized）· bundle 的 `app-update.yml` 写着 `channel: 1`，通用 Electron 源拿 `1-mac.yml` 会 404，所以用 recipe 读 `latest-mac.yml` · 无 changelog · 2026-09-16
...
- [x] [**Cua Driver**](com-trycua-driver.md) · `com.trycua.driver` — G (one-click `darwin-universal.tar.gz`) · **本机装了真包、钉旧版 0.28.1 → `duo install` 升回 0.28.2 走通**（inode 变、Team YCK386LBJ7 不变、`--deep --strict` 与 `spctl` 各退 0、厂商自己的 `cua-driver doctor` 六项全绿）· ⚠️ **stable tag 全部被厂商标成 prerelease**（monorepo 的 "Latest" 指针之争，厂商在 release 正文里写明），`/releases/latest` 会答成 `sandbox-v0.8.0` —— **必须 `usePrereleases`** · ⚠️ nightly tag **内含**一个完整 stable tag，不锚就每夜读出一个没发布的版本 · changelog 走内联解析（`.gitHubReleases` recipe 在这里恒得 0 条目，prerelease 闸）· `listPageSize: 25`（下限 19 实测）· **nightly 轨也已接**：轨道在磁盘上不可分辨（三份真包的 short/build 全是裸 base），唯一信号是 `~/.cua-driver/release-channel`，`CuaDriverChannel` 是本 registry 第一个读普通文本文件的 binding；nightly rule 只能捕 base 版本，**同 base 内的后续 nightly 看不见（实测 52% 的 nightly 才会提示）**；分叉态（意图 nightly / 装的 stable）端到端验过，duo 把它带回 nightly 轨 ✓ · 2026-09-16
- [x] [**Tinycast**](com-tinycast-app.md) · `com.tinycast.app` — G(stable/beta，**detection-only**) · beta 是独立 bundle id `com.tinycast.app.beta`，tag `-beta.N` 锚死、不收 stable tag · 真包 0.10.23 + 0.11.1-beta.96 zip 解包验证 ✓ · 两轨自签名、无 Team ID、`spctl` 拒 → **一键不可** · changelog 走 GitHub release body · 2026-09-17
...
- [x] [**Blender**](org-blenderfoundation-blender.md) · `org.blenderfoundation.blender` — C (`{majorMinor}` 模板) + Homebrew
