...
- **修完复审的发现,再对「修复那份 diff」审一轮,直到某一轮不再出现新的确证发现。** 上一条写的是
  「跑一遍」,跑一遍就满足了它的字面,而修复本身就是缺陷来源:修的时候最自信、审得最松,补丁又打在
  刚被证明容易出错的那个文件上。**普通 `/code-review` 和对抗复审同样适用。**
  2026-09-03 #314 五轮对抗复审共 13 条确证缺陷,**4 条是前一轮的修复引入的**(每轮 6→4→3→1;
  这组和下面 #315 的数字转引自当时的私人记录,复审是本地 subagent 跑的,PR 上查不到,未复测)。
  ...
  **只对 c01be6f1 单独跑的第二轮才抓到它**,修复是 f943f95c。而我那次在第一轮之后就停了,是被追问才补跑的。
  ...
  - **收敛看发现落在哪一层,不看条数。** #315(2026-09-03,给 `App/Sources` 加测试 target)三轮 19 条:
    第一轮 2 条在被测代码、4 条在 harness 和断言,第二、三轮**被测代码零发现**、13 条全在 harness 和
    措辞里。**连续两轮的发现都不在被测代码里,就可以收工**,哪怕这两轮还有 harness 或措辞上的发现;
    反过来,只要还有发现落在被测代码里就没到底,条数再少也一样——#660 第二轮那条就落在被测代码里。
