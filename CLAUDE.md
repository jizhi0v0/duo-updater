...
- **复审一律派 subagent 做,写 PR 的那个会话自己不审。** `/code-review` 和下面的对抗复审都算。
  在写代码的会话里直接调 `/code-review`,审的人手里攥着作者的全部推理和前提,
  而这个仓库的缺陷恰恰多半长在那些前提里——用户的观察是这样调用「很大概率发现不了问题」(没量化过)。
  非 fork 的 subagent 从空白上下文起步,**看不到父会话历史,但会加载 CLAUDE.md 全套**
  ([官方文档](https://code.claude.com/docs/en/sub-agents);2026-09-17 在本仓库探针实测:
  看不到父会话要求改什么、工具里有 `Skill`、清单里有 `code-review`)。
  - **用 `general-purpose`**(或自定义 agent),**别用 fork**——fork 继承父会话上下文,等于没换人;
    也别用 Explore / Plan——它们一次性、拿不到 agent ID 续不上,还不加载 CLAUDE.md。
  - **brief 只给「审什么」,不给「我为什么觉得对」**:PR 号或提交范围、要跑的
    `/code-review high`(对抗复审则是编号攻击面清单)。设计理由、自己排除过的风险别往里塞——
    那正是要让它独立重新推一遍的东西。「顺便告诉我 X」「特别看看 Y」也算塞:点名的地方就是作者自己
    改过、心里有数的地方,reviewer 的注意力会跟过去。要问它环境里的事(工具列表之类),等它交了结论再单独问。
  - **brief 里写明**:不跑构建和测试(会和你撞)、不改工作树(不带 `--fix` / `--comment`)、
    **不调 `ReportFindings`,把每条 finding 连同 `file:line`、失败场景、证据写进最终回复**。
    ...
    另外,2026-09-17 那个 subagent 两轮都报告自己的工具列表里**没有** `ReportFindings`(同一个 agent,
    只算一次观测)。如果 subagent 本来就拿不到它,这条防的是不会发生的事——照做无害,但别把它当成已证实的风险。
  ...
  - **在 subagent 里调 `/code-review`,审查可能跑在另一个隔离运行里,subagent 交回的是转述**
    (2026-09-17 观测一次,subagent 自述;它说 skill 原文是那次运行转述给它的)。所以 brief 里要它
    **逐条标明哪些是自己对着文件核过的、哪些是转述**。下面「原文」指的是 subagent 的最终回复,
    不是审查运行本身的输出;针对性复查续上的也是 subagent,不是那次审查运行。
  - **记下它的 agent ID / 名字**,针对性复查要回到它(见下面的流程)。

- **高风险改动上一次对抗复审,brief 明写"找我漏的"。** 风险看**波及面**不看行数:十行改在所有请求都要走的路径上,大过两百行改在一个功能里。派一个更强的模型(按上一条派 subagent,`model` 参数指定),给它编号的攻击面清单(没启用新开关的那些路径会不会回归、编码与边界、新选项和既有选项的未记录交互、静默失效面、诊断信息会不会开始说假话),并禁止它跑构建和测试(会和你撞)。**转发它的结论前先自己核一条**——#78 那次,一条看着像缺陷的(`VendorAppcastDeltas.patches` 仍读整个 body)其实是对的:它解析的是整份 Sparkle 文档,切片喂进去根本解析不出来。核完结论从"这是 bug"变成"这是对的,但缺注释,下一个人会把它改坏"。
...
- **复审之后按这个流程收尾:修过阻塞问题就要复查,但不追求某一轮零 finding。**

  ```
  提交 PR → subagent 跑 /code-review high → 裁定问题 → PR comment 留底 → fix
         → CI 通过 + 同一个 subagent 针对性复查 → PR comment 留底 → 合并
  ```

  - **裁定**:逐条核,只修确认成立的**阻塞问题**——不修的话,合并之后会做错事,或者让人照着一句错话做事
    (这个仓库里 CLAUDE.md、skill、注释的错误断言也算)。风格和可选重构不算阻塞。
  - **针对性复查**:修完之后用 `SendMessage` **续上那个审过的 subagent**(它保留完整的上一轮历史,
    包括自己提过什么;官方文档写明完成后的 subagent 可以这样续,主会话压缩不影响它的 transcript,
    默认保留 30 天),消息里附上修复的提交 SHA 和裁定结果(哪几条确认、哪几条驳回及证据),这样要求:
    ...
    **两点 diff `git diff <上轮 head>..<新 head>` 只在分支线性前进时等于「这次修复」**。两轮之间 rebase 到
    ...
    这段增量范围贴进 brief。
    **别在自己的会话里复查**——修的那个人最自信,理由同上。
    只有复查又确认了新的阻塞问题,才继续「修复 → 复查」。修复规模很大、改变了设计,**新派一个** subagent
    跑完整的 `/code-review high`——续旧的会带着它上一轮的视角。
    很小的修复,如果已有回归测试证明「修复前失败、修复后通过」且影响范围清楚,可以不额外跑 AI 复查。
  - **每一轮(首审和每次复查)都发一条 PR comment 留底**,`gh pr comment <n> -F <文件>`
    (别用 `-b`,zsh 会吃掉反引号)。内容三块:
    1. 头信息:第几轮、reviewer 是新派的还是续上的同一个、**审的是哪个 head SHA**。agent ID 不写:
       它只在本机这个会话里能用来续上,写进公开 PR 没人用得上,而且 Agent 工具的返回里明写别把它贴进对外回复;
    2. **reviewer 最终回复原文**,不改写——作者转述就又把作者的视角塞回去了。**先把收到的回复原样存成文件,
       再拼进 comment**,别手抄、别翻译:这一步没有闸查,2026-09-17 第一次发出去的就是作者转写成中文的版本。
       唯一允许的改动是脱敏:
       仓库是公开的,而 reviewer 读得到本机文件和 `~/.claude/CLAUDE.md`,本机绝对路径、本机装了什么/
       在跑什么这类机器状态、token 一律换成 `[已脱敏]`;
    3. 作者裁定:每条确认 / 驳回,驳回附证据;reviewer 判的阻塞和作者裁的不一致时,两边并列写。

    为什么要落在 PR 上:subagent 的 transcript 只在本机、默认 30 天后删;PR 合进 main 后通常只剩一个提交,
    中间提交要 `git fetch origin pull/<n>/head` 才取得回(见下面 #660 那条),而提交里本来就没有裁定。
    「阻塞项闭环了」这件事没有闸查,**这条 comment 就是它唯一的证据**。
  - **非阻塞项默认只留在那条 comment 里,不逐条开 issue。** 只有同时满足三条才开 issue、并把链接补进 comment:
    是真缺陷或缺失的防护(不是风格、不是可选重构)、不在本 PR 范围内、不修会有具体后果(写得出失败场景)。
  ...
  - **为什么修复要复查**:修的时候最自信、审得最松。2026-09-03 #314 多轮对抗复审共 13 条缺陷,其中 4 条在
    ...
    header 让 `current` 停在上一个 target 上),**只对 c01be6f1 单独复查才抓到**,修复是 f943f95c。
    (这些提交没进 main,#660 是 squash 合的;要看就 `git fetch origin pull/660/head`。)
  - **为什么不追求零 finding**:#661(写这条规矩的 PR)先按「某一轮没有新发现才收敛」跑,十轮还没停,
    逐轮记录在它的 PR 描述里。对抗式复审拿去审文字,总能挑出东西;停止条件要落在「阻塞项闭环」上。
  - **`/code-review` 不是合并门槛,「每个 PR 合并前跑一遍」和这套流程都只靠自律。** `main` 的 ruleset 要求走 PR、
    0 个 approval、必需检查只有 `test`(strict=false):CI 一绿就可合,审没审、复查没复查都不影响。
    所以**阻塞项闭环之前别开 `--auto`**,已经开了就 `gh pr merge <n> --disable-auto`。2026-09-07 #401:
    我 03:22:10Z 开了 `--auto`,03:29:00Z CI 一绿就合了,复审改出来的修正 03:34:24Z 才提交(见「CI」一节)。
