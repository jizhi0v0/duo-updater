...
- **复审一律派 subagent 做,写 PR 的那个会话自己不审。** `/code-review` 和下面的对抗复审都算。
  ...
  而这个仓库的缺陷恰恰多半长在那些前提里——用户的观察是这样调「很大概率发现不了问题」(没量化过)。
...
- **复审之后按这个流程收尾:修过阻塞问题就要复查,但不追求某一轮零 finding。**
  ...
  - **针对性复查**:修完之后用 `SendMessage` **续上那个审过的 subagent**(它保留完整的上一轮历史,
    ...
    续不上(ID 丢了、被手动停掉、过了保留期)就新派一个,把上一轮的 finding 原文和裁定贴进 brief。
  ...
  - **每一轮(首审和每次复查)都发一条 PR comment 留底**,`gh pr comment <n> -F <文件>`
    ...
    2. **reviewer 最终回复原文**,不改写——作者转述就又把作者的视角塞回去了;
    3. 作者裁定:每条确认 / 驳回,驳回附证据;reviewer 判的阻塞和作者裁的不一致时,两边并列写。

    为什么要落在 PR 上:subagent 的 transcript 只在本机、默认 30 天后删;PR 是 squash 合的,
    中间提交也看不到(见下面 #660 那条)。「阻塞项闭环了」这件事没有闸查,**这条 comment 就是它唯一的证据**。
