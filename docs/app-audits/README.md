...
- [x] [**Windscribe**](com-windscribe-client.md) · `com.windscribe.client`（**不是 cask 写的 `com.windscribe.gui.macos`**）— P(stable, detection-only) + C · 版本读厂商 `api.windscribe.com/ChangeLogs/summary` 的 `release_full_version`，要一个 `Authorization` 头但值不校验（存在性检查）· **不用** `CheckUpdate` 的 `beta=` 参数——0/1/2/3 返回逐字节相同，语义证不出来，而 prerelease 领先 stable 是这家 75% 的常态（819 天里 618 天）· `release_version` 只有两段（`2.24`），读它是永久降级；`(?:(?!"platform")…)` 边界防跨条目 · changelog 走 GitHub releases（`.gitHubReleases`，无需 header，实测 9 条）· **GitHub 也能做版本源**（`/releases/latest` 天然只给 stable，实测 19/19 对、51/51 不漏），没用它是因为它缺了 1 个 release 轨版本（2.15.9），会安静地少报一版 · 一键**结构性拒绝**：dmg 里只有安装器壳，真身在 `.tar.lzma` 里，安装要写 LaunchDaemon + 特权 helper + system extension · beta/guinea pig 无法检测（Pattern D，见 §3），后果是那两轨用户被提供 stable 更新，已接受并说明 · 未装机审计，从官方 dmg 解包 + `channel-verify` 生产全链验证 · 2026-09-07
