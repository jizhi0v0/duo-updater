...
- [x] [**Carbon Copy Cloner**](com-bombich-ccc.md) · `com.bombich.ccc` — P(stable/beta，detection-only) · 有 `SUFeedURL` 但该端点对任何请求都回空 body（原生 `URLSession` 复现），Sparkle 源静默失效；改读 cask 自带 livecheck 同款的 `download_ccc.php?v=latest` 重定向文件名 · cask 是 `auto_updates` · beta 用 `?v=latestbeta`（无连字符，此前误以为需要抓包才能拿到，2026-08-29 补齐）· channel 信号是版本串 `-b<N>` 短后缀，`ReleaseChannel.detect()` 新增 step 0.8 · 2026-08-29
