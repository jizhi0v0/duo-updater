...
- **复审一律派 subagent 做,写 PR 的那个会话自己不审。** `/code-review` 和下面的对抗复审都算。
  ...
  - **brief 里写明**:不跑构建和测试(会和你撞)、不改工作树(不带 `--fix` / `--comment`)、
    **不调 `ReportFindings`,把每条 finding 连同 `file:line`、失败场景、证据写进最终回复**。
    父会话只看得到它的最终回复,而 `ReportFindings` 的工具说明要求调了它就不要再把 findings 打成文本
    (2026-09-17 读的是工具说明原文;subagent 实际照 brief 还是照工具说明,未验证)。
    照工具说明走,正文里就没有 findings,下一条要求的第一行 `阻塞项:无` 会把它伪装成「真没有」。
...
- **复审之后按这个流程收尾:修过阻塞问题就要复查,但不追求某一轮零 finding。**
  ...
  - **每一轮(首审和每次复查)都发一条 PR comment 留底**,`gh pr comment <n> -F <文件>`
    ...
    1. 头信息:第几轮、reviewer 的 agent ID、**审的是哪个 head SHA**;
    2. **reviewer 最终回复原文**,不改写——作者转述就又把作者的视角塞回去了。唯一允许的改动是脱敏:
