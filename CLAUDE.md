...
- **修完复审的发现,再对「修复那份 diff」审一轮,直到收敛(判据见下)。** 上一条写的是「跑一遍」,
  ...
  2026-09-03 #314 多轮对抗复审共 13 条确证缺陷,其中 4 条在 `App/Sources`,**这 4 条里 3 条是修上一条时
  ...
  **只对 c01be6f1 单独跑的第二轮才抓到它**,修复是 f943f95c。而我那次在第一轮之后就停了,是被追问才补跑的。
  ...
  - **收敛看发现落在哪一层,不只看条数。** 满足任一条就可以收工:(1) 某一轮没有新的确证发现;
    (2) 连续两轮的确证发现**全在措辞、注释、文档里**,没有一条落在会执行的代码上——产品代码、测试、
    fixture、脚本、闸都算。这两轮留下的措辞问题照样要改;那份修复只碰措辞就不必再审,碰到会执行的代码
    就还得再审一轮。反过来,只要还有发现落在会执行的代码上就没到底,条数再少也一样:#660 第二轮那条就在
    `test_targets()` 里。
    ⚠️ **别按「被测代码 vs harness」划层,#315 就是反例。** 它的 PR 描述说第二、三轮的发现全在 harness
    和措辞里、`ScanRowAssembly` 连续两轮干净,但那两轮抓到的有防空过闸的缺陷,而那道闸
    (`app-tests.sh` / `app_test_coverage.py`)正是 #315 要交付的东西;第三轮还是**只对 harness 审的**,
    `ScanRowAssembly` 那一轮根本没被看。**范围收窄过的一轮,不能算被排除在外那部分的干净轮。**
  - **`/code-review` 不是合并门槛,这条和上一条都只靠自律。** `main` 的 ruleset 要求走 PR、
    ...
    我 03:22:10Z 开了 `--auto`,03:29:00Z CI 一绿就合了,复审改出来的修正 03:34:24Z 才推(见「CI」一节)。
