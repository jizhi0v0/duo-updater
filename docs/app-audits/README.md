...
- [x] [**Qoder CN IDE**](com-aliyun-lingma-ide.md) · `com.aliyun.lingma.ide` — P C (one-click arm64 zip, `versionTemplate`) · 国内版 IDE，与 Qoder IDE 同源同 commit，但 Team `9DFNGU9AK5`、更新服务器、说明页都分开 · 走 `/api/qodercn/update/…/latest`（抓包得到；VS Code 原路径是 Lingma 的旧表）· ⚠️ 答案逐请求抖动（同国际版）· 回答里的 `lastest` zip 在 CDN 边缘滞后一版，故按版本拼地址 · 2026-09-22
