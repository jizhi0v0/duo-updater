...
## channel-verify 状态
- ✓ **已验证 2026-06-04**（`--scan` 跑生产 `AppScanner`→`ChannelBinding`，本机装的就是 `~/Applications/DuoPaste.app`）。`sparkleIncludePrereleases=1` 且装的是 `…-beta` 构建 → **beta**；appcast 当前仅 `<sparkle:channel>beta` 项，head=0.1.1251-beta=installed。VendorProbe 故意无应答（机制是 channel-tag Sparkle）。**stable 也已本机验证**（临时翻 `sparkleIncludePrereleases=false`→`--scan`=stable→还原为 1）。证据：`application-test/records/io-duopaste-daemon.md`
