...
@Suite struct BackupStoreDestinationTests {
    ...
    private func withStores(
        ...
        _ body: (_ outbox: URL, _ destination: URL) async throws -> Void
    ) async throws {
        ...
        try await BackupStore.$rootOverride.withValue(outbox) {
            try await BackupStore.$destinationOverride.withValue(configured) {
                try await body(outbox, destination)
            }
        }
    }
    ...
    private func twoStores(_ outbox: URL, _ destination: URL) async throws -> URL {
    ...
    /// watching it fail.
    @Test func verifyCatchesATamperedArchive() async throws {
        try await withStores { outbox, destination in
            _ = try await twoStores(outbox, destination)
    ...
    @Test func verifyCatchesAChangedBundleInTheOutbox() async throws {
        try await withStores { outbox, destination in
            _ = try await twoStores(outbox, destination)
    ...
    @Test func aBackupWithNothingRecordedIsNeitherPassNorFail() async throws {
        try await withStores { outbox, destination in
            _ = try await twoStores(outbox, destination)
    ...
    @Test func deepVerificationCatchesWhatTheDigestCannot() async throws {
        try await withStores { outbox, destination in
    ...
    /// those.
    @Test func verifyIgnoresDirectoriesThatAreNotBackups() async throws {
        try await withStores { outbox, destination in
            _ = try await twoStores(outbox, destination)
    ...
    @Test func withoutADestinationTheStoreIsLocalOnly() async throws {
    ...
    @Test func aMountedDirectoryWithAMatchingMarkerIsReady() async throws {
        try await withStores { _, destination in
    ...
    @Test func anUnmountedDestinationIsRefusedAndNothingIsCreated() async throws {
    ...
    @Test func aDirectoryWithoutOurMarkerIsNotOurDisk() async throws {
        try await withStores(destinationIdentity: "expected", markerIdentity: .none) { _, destination in
    ...
    @Test func anotherDiskAtTheSamePathIsReportedAsAMismatch() async throws {
        try await withStores(destinationIdentity: "expected", markerIdentity: .some("actual")) { _, _ in
    ...
    /// rollback point.
    @Test func saveStillSucceedsIntoTheOutboxWhenTheDiskIsMissing() async throws {
    ...
    @Test func aBackupOnTheDestinationIsFound() async throws {
        try await withStores { outbox, destination in
            let apps = outbox.deletingLastPathComponent().appendingPathComponent("apps")
            try FileManager.default.createDirectory(at: apps, withIntermediateDirectories: true)
            let app = try makeApp(named: "App.app", in: apps, marker: "v1")
    ...
    @Test func theOutboxWinsWhenACopyExistsInBothStores() async throws {
        try await withStores { outbox, destination in
            let apps = outbox.deletingLastPathComponent().appendingPathComponent("apps")
            try FileManager.default.createDirectory(at: apps, withIntermediateDirectories: true)
            let app = try makeApp(named: "App.app", in: apps, marker: "v1")
    ...
    @Test func removingABackupDropsBothCopies() async throws {
        try await withStores { outbox, destination in
            let apps = outbox.deletingLastPathComponent().appendingPathComponent("apps")
            try FileManager.default.createDirectory(at: apps, withIntermediateDirectories: true)
            let app = try makeApp(named: "App.app", in: apps, marker: "v1")
    ...
    /// gone.
    @Test func anUnreachableDestinationStillListsTheOutbox() async throws {
        try await withStores { outbox, destination in
            let apps = outbox.deletingLastPathComponent().appendingPathComponent("apps")
            try FileManager.default.createDirectory(at: apps, withIntermediateDirectories: true)
            let app = try makeApp(named: "App.app", in: apps, marker: "v1")
    ...
    @Test func aBackupIsRestoredFromTheDestinationArchive() async throws {
        try await withStores { outbox, destination in
            let apps = outbox.deletingLastPathComponent().appendingPathComponent("apps")
            try FileManager.default.createDirectory(at: apps, withIntermediateDirectories: true)
            let app = try makeApp(named: "App.app", in: apps, marker: "v1")
    ...
    @Test func aTamperedDestinationArchiveIsRefused() async throws {
        try await withStores { outbox, destination in
            let apps = outbox.deletingLastPathComponent().appendingPathComponent("apps")
            try FileManager.default.createDirectory(at: apps, withIntermediateDirectories: true)
            let app = try makeApp(named: "App.app", in: apps, marker: "v1")
            ...
            await #expect(throws: BackupStore.BackupError.self) {
                _ = try await BackupStore.restore(forKey: "k", over: app)
            }
