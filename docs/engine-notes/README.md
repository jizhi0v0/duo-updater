...
- [`homebrew-cask-catalog.md`](homebrew-cask-catalog.md) — `DuoUpdaterCore/Sources/DuoUpdaterCore/Sources/HomebrewCaskCatalog.swift`
- [`recipe-coding.md`](recipe-coding.md) — `DuoUpdaterCore/Sources/DuoUpdaterCore/Sources/RecipeCoding.swift`: why recipes stay Swift (step 3 of the recipe refactor, 2026-09-15), the options and measurements, when to reopen, and the constraints a remote patch format would have
- [`self-updater-stash.md`](self-updater-stash.md) — `DuoUpdaterCore/Sources/DuoUpdaterCore/Sources/SelfUpdaterStash.swift`: installing from an installer the app's own updater already downloaded — §1 the electron-updater layout and how to re-run the survey, §2 why the version gate carries the feature, §3 the attribution gate and the two copies that defeat every other check, §4 the dmg/pkg shapes left unimplemented and why, §5 what reading a version out of a zip costs, §6 what the substitution switches off, §7 why a refusal is logged louder than a skip, §8 the end-to-end run
