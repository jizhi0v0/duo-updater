...
- **修完复审的发现,再对「修复那份 diff」审一轮,直到收敛:某一轮不再出现新的确证发现,或者连续两轮的
  发现都不落在被测代码里(见下)。** 上一条写的是
  「跑一遍」,跑一遍就满足了它的字面,而修复本身就是缺陷来源:修的时候最自信、审得最松,补丁又打在
  刚被证明容易出错的那个文件上。**普通 `/code-review` 和对抗复审同样适用。**
  2026-09-03 #314 五轮对抗复审共 13 条确证缺陷(#315 的 PR 描述有公开记录,那里和「App 层的测试 target」
  一节写的「4 条在 `App/Sources`、其中 3 条是修复引入的」只算 `App/Sources` 那个子集);按全部 13 条算,
  **4 条是前一轮的修复引入的**,每轮 6→4→3→1——这组全量数字只在当时的私人记录里,未复测。
  ...
  **只对 c01be6f1 单独跑的第二轮才抓到它**,修复是 f943f95c。而我那次在第一轮之后就停了,是被追问才补跑的。
  ...
  - **收敛看发现落在哪一层,不看条数。** #315(2026-09-03,给 `App/Sources` 加测试 target)三轮复审,
    PR 描述里写着:第二、三轮的发现**全在 harness 和我自己的措辞里**,`ScanRowAssembly` 连续两轮干净。
    **连续两轮的发现都不在被测代码里,就可以收工**,哪怕这两轮还有 harness 或措辞上的发现;
    反过来,只要还有发现落在被测代码里就没到底,条数再少也一样。
    ⚠️ **「被测代码」指这个 PR 要交付的东西,不按文件类型分。** #315 交付的是 `ScanRowAssembly`,
    为验证它写的用例、fixture、`app-tests.sh` 的闸才是 harness;#660 交付的就是
    `check_localizable_keys.py` 这道闸本身,它是被测代码——第二轮那条落在这里,不能因为「它是个检查脚本」
    记成 harness 然后收工。
  - **`/code-review` 不是合并门槛,这条和上一条都只靠自律。** `main` 的 ruleset 要求走 PR、
    0 个 approval、必需检查只有 `test`(strict=false):CI 一绿就可合,审没审、审了几轮都不影响。
    所以**循环没收敛之前别开 `--auto`**,也别因为看到绿勾就合:修复提交的 CI 可能在下一轮复审
    跑完之前就绿了。2026-09-07 就是合并比复审的修正早了 324 秒(见「CI」一节)。
...
- ⚠️ **复审发现问题就先说,别攒一轮再推。** 2026-09-07 实测的时间线:be36b32 的 run 在
  ...
  **这次合并完全正确,绿了就是可合的**。而我把复审改出来的修正提交在 **03:34:24Z**,
  **比合并晚 324 秒**:那四条缺陷在被合进去的那一刻,一条都还没说出口。得靠 #403 补。
  **缺陷没被说出口的每一分钟都是它自己的时间窗**,而窗口什么时候关不由我决定,
  所以能做的只有把话说在前面——攒一轮再推是在赌一件我说了不算的事。
