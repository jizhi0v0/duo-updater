...
**“Download Traffic”窗口现在叫“网络”，并多了一个标签页。** “下载次数”还是你原来那份账本——每次更新作为文件花了多少。“请求”是新的：DuoUpdater 替你发出的每一次请求，为了什么、属于哪个 App、花了多少，并且带一个筛选框，让你可以向这份日志提问，而不是一路翻。顶部的数字回答的是你当前筛选出来的范围，不是历史总量。
...
**检查失败现在会明确说明失败，而不是显示空白。** 某个应用的版本只能由供应商自己的端点回答，它此前只有一种方式表示“无应答”：空白的破折号，这也是 DuoUpdater 根本不追踪的应用的显示方式。所以供应商端点超时或迁移，看起来会和“DuoUpdater 不追踪此应用”一模一样——显示为一个看似永久的结论，没有重试按钮，也不会出现在“App 无法检查”的计数里。现在这些情况会被算作失败的检查，这才是它们真实的样子：有重试按钮、计入统计，并说明原因。但这并不是要覆盖所有情况：你的 Mac 确实无能为力的那些——你所在的发布轨道没有对应的 recipe，或者端点需要这台机器没有的身份认证——仍然保持原来那个安静的破折号；一个连续失败三轮的检查，也仍然会从横幅里退出，而不会被永久钉在那里。由 JetBrains Toolbox 安装的应用，即便其背后的版本读取失败，也仍然保留“打开 Toolbox”按钮，因为不管怎样，打开 Toolbox 都是正确答案。

**“重新启动以应用”的提醒现在一旦你重新启动就会消失。** 当应用自己的更新器在后台下载了构建时，DuoUpdater 会通知你并在通知上提供重新启动按钮。使用它是有效的——应用会在新构建上重新启动并显示“现在运行的是 1.0”——但它替换的提醒仍然保留在通知中心下方，仍然要求你重新启动刚才已经重新启动过的内容。DuoUpdater 在等提醒停止相关的时候检查，但检查的时刻恰好是已经被考虑过的唯一时刻。现在只要没有未完成的重启，它就会移除提醒，这也清理了 DuoUpdater 未运行时应用自己应用更新而留下的提醒。
