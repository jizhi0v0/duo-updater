...
**这不是审计**：family `com-1password-1password`（`Recipes/com-1password-1password.swift`）里 1Password 8 `com.1password.1password` 的覆盖情况没有审过。这份文件只接收从 recipe 注释迁出的历史，登记在索引的「仅迁出历史（未审计）」一节。
