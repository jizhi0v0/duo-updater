...
**这不是审计**：family `com-crystalidea-macsfancontrol`（`Recipes/com-crystalidea-macsfancontrol.swift`）里 Macs Fan Control `com.crystalidea.macsfancontrol` 的覆盖情况没有审过。这份文件只接收从 recipe 注释迁出的历史，登记在索引的「仅迁出历史（未审计）」一节。
