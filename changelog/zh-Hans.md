...
**现在支持 Qoder CN 和 Qoder CN IDE 了。** Qoder 国内版的桌面应用和 IDE 都能检查更新、一键安装，更新说明也会显示在窗口里。
...
**应用自己下载的更新过期了，现在也能再更新。** 如果应用自带的更新器下载了某个版本，之后又出了更新的版本，DuoUpdater 现在会先清掉旧的下载，再装最新版。以前「更新」一直提示现在安装会被撤销，那一行也一直不变。

**`duo install` 会告诉你最新版已经在等着安装了。** 如果应用自带的更新器已经下载好了最新版本，它现在会直接说明，并建议退出该应用或运行 `duo restart`。以前它会错误地说这个应用只能检查、不能安装。

## 0.4.1

**搜狗输入法 现在可以一键更新了。** DuoUpdater 会按照这个输入法自己更新的方式来装，装好的应用留在原处；而且会先把你的词库和设置复制一份，所以回滚的时候它们也会一起回来。
...
**「Requests」标签页现在会说明自己记了什么、没记什么。** 它记录的是 DuoUpdater 自己发出的请求。发布说明页面会自己去取图片和字体，App Store 和 Homebrew 的更新由各自的工具完成——这些都不会出现在里面，现在窗口会直说，而不是留给你去猜。
...
**DuoUpdater 自己的更新现在会记在它自己名下。** 它的版本检查、发布说明和下载，在「Network」窗口里的 App 一列一直是空的。

**内部改进。** 藏在网址路径里的凭据，现在会在请求被记录之前去掉，就像查询串里的凭据一直以来那样。

## 0.3.83

**「Download Traffic」窗口现在叫「Network」，并多了一个标签页。** 「Downloads」还是你原来那份账本——每次更新作为文件花了多少。「Requests」是新的：DuoUpdater 替你发出的每一次请求，为了什么、属于哪个 App、花了多少，并且带一个筛选框，让你可以向这份日志提问，而不是一路翻。顶部的数字回答的是你当前筛选出来的范围，不是历史总量。
...
**从菜单栏里打开的东西，现在第一次点击就能打开。** 在某一行上选"更新说明"——或者设置、发布记录——第一次完全没有反应，从第二次点击起才正常。重新打开菜单会让每一次尝试都重新变成"第一次"，所以对于打开菜单、点一下、然后期待窗口出现的人来说，它从来就没成功过。菜单里的第一次点击被拿去把 DuoUpdater 切到最前面了，而不是去做它本来要做的事。打开的窗口现在也会被按在最前面：它此前会被抬上来又在同一瞬间被压回去，那是这件事看起来"点了没反应"的另一种成因。
...
**百度网盘现在可以追踪了——支持版本检测、一键更新和窗口内的发布说明。** 常规渠道都不可用。它没有 Sparkle feed；通过 Homebrew cask 也无法追踪手动安装的副本；其自身 bundle 内引用的更新清单也已经失效——该清单文件、对应的 arm64 版本，以及存放它们的整个目录，全都返回 404。DuoUpdater 改为读取供应商下载页面所调用的端点，并安装该端点返回的经过公证的 arm64 磁盘映像。发布说明来自同一个地方："版本更新"页面有 Mac版 标签页，但页面本身是空的，通过 API 填充内容，DuoUpdater 直接读取 API 并在自己的窗口中渲染最新 40 个发布。值得说清楚的限制：该 feed 只发布你看到的版本号，不包括底层的构建号，所以保持版本名称不变的一次重新打包，在这里是看不见的。它无法创造不存在的更新——只是无法看到这一种情况。

**检查失败现在会明确说明失败，而不是显示空白。** 某个应用的版本只能由供应商自己的端点回答，它此前只有一种方式表示"无应答"：空白的破折号，这也是 DuoUpdater 根本不追踪的应用的显示方式。所以供应商端点超时或迁移，看起来会和"DuoUpdater 不追踪此应用"一模一样——显示为一个看似永久的结论，没有重试按钮，也不会出现在"应用无法检查"的计数里。现在这些情况会被算作失败的检查，这才是它们真实的样子：有重试按钮、计入统计，并说明原因。但这并不是要覆盖所有情况：你的 Mac 确实无能为力的那些——你所在的发布轨道没有对应的 recipe，或者端点需要这台机器没有的身份认证——仍然保持原来那个安静的破折号；一个连续失败三轮的检查，也仍然会从横幅里退出，而不会被永久钉在那里。由 JetBrains Toolbox 安装的应用，即便其背后的版本读取失败，也仍然保留"打开 Toolbox"按钮，因为不管怎样，打开 Toolbox 都是正确答案。

**"重新启动以应用更新"的提醒现在一旦你重新启动就会消失。** 当应用自己的更新器在后台下载了构建时，DuoUpdater 会通知你并在通知上提供重新启动按钮。使用它是有效的——应用会在新构建上重新启动并显示"现在运行 1.0"——但它替换的提醒仍然保留在通知中心下方，仍然要求你重新启动刚才已经重新启动过的内容。DuoUpdater 在等提醒停止相关的时候检查，但检查的时刻恰好是已经被考虑过的唯一时刻。现在只要没有未完成的重启，它就会移除提醒，这也清理了 DuoUpdater 未运行时应用自己应用更新而留下的提醒。

## 0.3.70

**在一个版本号下发布许多构建的应用现在在任何地方都被正确处理了。** Mac 应用带有两个版本字符串：你看到的（"1.0"）和实际计数的构建号。大多数应用两个都更新。有些只更新构建：Amp 在一天内发布了十个构建，每个都叫 1.0；Surge 以 6.9.0 发布了四个独立的版本；JetBrains 的预览构建也是如此。DuoUpdater 在大约十几个地方通过比较*显示的*版本来判断"这个改变了吗"，而对于这些应用，该比较只能永远回答"没有"——或者在询问"这些相同吗"时，"是的"——无论实际发生了什么。接下来是这次错误的后果。这是一个错误，因为 Amp 让它暴露了。

**重新启动不再看起来挂起三分钟然后报告没有发生的失败。** 在一个自身更新器已经准备好构建、正在等待的应用上点击"重新启动"，此前会旋转三分钟，然后提示失败——而更新实际上已经在一两秒内完成应用，应用也已经在新构建上重新打开。DuoUpdater 当时是在等待"显示版本"发生变化，而这个变化永远不会到来。在 Amp 上实测：189 秒的旋转，对应的是一次实际耗时不到一秒的交换。现在它在一秒以内就会结算，而真正的失败仍然会被如实报告。

**重新启动不再提供已经过时的构建。** 当应用的更新器下载了一个构建而开发者随后发布了另一个时，DuoUpdater 仍然提供重新启动——所以你重新启动并立即落后一个构建，这正是那个检查被写来防止的。这些行现在提供**更新**代替，它会获取当前构建。

**跳过版本不再永久沉默应用。** 这是最值得留意的一条。"跳过此版本"的本意是拒绝一个发布，让下一个通过。它此前只记录显示的版本，所以对于在不同构建之间保留同一个版本名称的应用，跳过一次就会拒绝*每一个*未来的发布——永久生效，重启后依旧，屏幕上也没有任何提示说明这个应用已经被静音。跳过现在会记录它拒绝的那个构建。修复带来一个后果：一个较早版本的 DuoUpdater 跳过过的版本，会再次被提供给你，因为旧的记录说不清它当初指的是哪个构建。再跳过一次，就会被正确记录下来。

**回滚功能，为那些看起来什么都没改变的更新重新出现了。** 工作台会隐藏掉一个不会有任何效果的回滚选项——而对这些应用来说，每一次回滚看起来都不会有效果，所以真实更新完成后这一行就消失了，一份完整的备份却还静静躺在磁盘上，没有办法触达。

**重新启动的提醒重新变回每个构建只宣布一次，并且不会因为开发者又发布了一个而被打断。** 0.3.69 把一个每五分钟重复一次的提醒，换成了一个针对每个分阶段构建只说一次的提醒——但它是靠构建名称来识别构建的，所以对这些应用来说，它只宣布了第一个，之后的每一个都保持沉默。这个问题已经修复。另外：如果应用因为有未保存的工作而拒绝退出，DuoUpdater 会记下来，等你处理完之后再重新打开它。此前，如果开发者在你还没做决定的时候又发布了一个新构建，这条记录就会被直接丢弃，导致更新完成后应用被关着、却没有东西能把它重新打开。现在它会跟着新的构建走。
...
**在更新它们任何一个之前，它们学到的一切都会先被复制一份。** 你的词典、你的设置和你的账户状态并不保存在应用本体里，所以 DuoUpdater 原本保留的回滚副本无法覆盖到这些内容。现在，每次输入法更新之前都会先给它们拍一次快照，和回滚副本一起放回去，而且这份快照几乎是免费的——其中一个应用 578 MB 的数据，只需要十分之一秒、几乎不占用额外磁盘空间。这是一张安全网，而不是一次修复：应用下次启动时打算怎么处理你的数据，运行的是它自己的代码，没有什么能够挡在前面。但现在至少有了可以恢复回去的东西。

**更新不再悄悄拿走一个应用自我更新的能力。** 当 DuoUpdater 需要以管理员身份替换一个应用时，它会恢复应用的所有者，却不会恢复应用安装时拥有的权限——而普通下载解压出来的权限，通常比安装程序设置的权限更窄。对这两款输入法来说，这就是它们自己的更新器能不能顺利完成下一次更新、能不能收拾好自己残局之间的区别。现在，应用安装时拥有的权限会被一并带到替换它的新副本上。

**DuoUpdater 不再每隔五分钟就要求你重新启动同一个应用。** 一个自我更新的应用，会把新版本留在磁盘上，等你退出它。DuoUpdater 此前每次检查都会注意到这一点，并发一次提醒，所以一个你已经决定暂不重启的构建，只要你留着不管它，就会一直缠着你。现在每个构建只会宣布一次：重新启动它、跳过它，或者干脆不理会这个应用，它就会安静下来，等下一个构建出现时再正常宣布。
...
**搜狗输入法现在被追踪了——靠的是询问它自己的更新器，而不是读取它的网站。** 搜狗的更新日志页面发布的是三段式版本号，而安装的副本实际是四段式，所以要跟它比较，就得先把真实的四段版本号裁剪掉一段。它自己的更新检查接口则直接用应用自身的版本编号作答。DuoUpdater 像搜狗自己一样查询这个端点，读取完整的四段版本号，所以哪怕一次只改动最后一段的重新打包，也能被发现。这一条只做到了检测：搜狗的更新器做的事远不止替换应用本身——它还会重新注册一个 QuickLook 生成器、把你的数据搬到新位置、并强制退出输入法——所以更新动作本身仍然交给它自己完成。有一点值得知道，这是搜狗自己的行为，不是我们造成的：他们的端点会根据发起请求的 macOS 版本来回答，运行 macOS 28 的 Mac 如果直接问自己那个版本，得到的会是一个 2023 年的构建。DuoUpdater 不会替自己去问，所以不管在哪台 Mac 上，你看到的都是当前的最新版本。
...
**Android Studio 的预览渠道，现在不会再给你推送一个比你手上这个还旧的构建。** Google 是按发布顺序而不是按版本号来排列它的发布列表的，DuoUpdater 为此把这份列表分别检索了三次——一次找版本号、一次找日期、一次找下载地址——结果一个发布时间晚于某个更新金丝雀版本的候选版本，可能会把版本号取自一个条目、却把文件取自另一个条目。8 月 26 日，Canary 渠道给出的答案是 2026.1.4 RC 2，而这时 2026.2.1 Canary 2 其实早已发布。现在这些答案必须全部来自同一个条目。
...
**连续切换两次应用的发布渠道，不会再让这一行卡在旧的结果上。** 翻转一次渠道开关，会让 DuoUpdater 重新检查那个应用；如果在第一次检查还没跑完时又翻转了第二次，这第二次翻转过去会被直接丢弃——导致这一行继续向刚刚关闭 beta 的人推荐 beta 版本，直到某个不相关的事件恰好触发了另一次检查为止。现在较新的那次翻转会接管较旧的那次，而一次中途被打断的检查，会把它没来得及查到的应用标记为留给下一轮，而不是当成已经处理过记下来。翻转之后，这一行也会更快进入检查中的状态，这样屏幕上的答案已经被你的开关设置为过时、却还没更新过来的那段时间窗口，也就更短了。
...
**特定架构的更新，现在会挑选这台 Mac 真正能运行的那个构建。** 有几个应用会在同一份 Sparkle feed 里把同一个版本发布两次——一次给 Apple silicon，一次给 Intel——DuoUpdater 过去是靠哪个下载地址碰巧排在字母顺序前面来打破这个平局的。现在它会读取 feed 里标注的硬件要求，如果供应商没有填写，就改读文件名里的架构信息。本机构建现在总能胜出，这台 Mac 根本无法启动的构建也不会再被提供。GitHub 发布也享受同样的处理，不会仅仅因为最新工件面向另一种架构，就把一个其实完全健康的 recipe 判定为失效。

**ChatGPT 的更新检查现在会跟随账户所绑定的发布轨道。** OpenAI 有时会把商业版和企业版账户留在较旧的桌面构建上，而让新构建先推送给消费者账户。DuoUpdater 过去在这项检查里忽略了账户的套餐类型，结果就是悄悄给所有人都选了最保守的那条轨道：它可能会说已安装的版本不知怎么反而更新，尽管 ChatGPT 自己其实已经在下载更新的构建；也可能会推荐一个应用自己的更新器随后又会替换掉的构建。现在它会在应用自己发起的那个更新请求里，一并带上从 ChatGPT/Codex 登录状态读到的套餐标签。如果拿不到这个标签，就继续保持保守的行为；账户凭证本身则永远不会被放进请求或诊断信息里。

**一次 macOS 迟迟不回应的重新启动请求，现在不会再卡住之后的每一次更新了。** Launch Services 偶尔会接受重新打开一个已更新应用的请求，然后就再也没有回调。这一行此前会一直卡在"重新启动…"状态，它的"重新启动"按钮保持禁用，DuoUpdater 自己的更新也只能排在它后面等待。现在，一个发起一分钟后仍未得到回应的重启请求，会被直接判定为失败并释放，这样这一行就能恢复正常，更新器的其余部分也能继续工作。
...
**更新留下的说明文字，现在会自己清理干净。** 当 DuoUpdater 把一个应用移交给它自己的更新器时，这一行会说明这一点——"已将其带到前台，以便它自己的更新器应用更新"。这句话过去会一直留在那里：唯一能移除它的办法，就是在同一个应用上再启动一次更新——所以更新明明早就完成了，这行字却还挂在下面，描述着几个小时前就已经结束的事情。现在只要应用变成最新状态，它就会立刻消失。至于那条提示"更新已应用但没有回滚点"的警告，则被特意保留了下来，因为它描述的是已经发生过的更新，只有在更新结束之后才真正有意义。
...
**一条因更新失败而留下的说明，不会再比失败本身活得更久。** 当一次更新无法应用时——比如因为其他东西已经在安装——这一行会用红色文字说明原因。但此前从来没有什么东西会把这行字撤掉：即使更新真的完成了、这一行变回一个对勾，旧的说明仍然留在下面，而且会一直留到 DuoUpdater 重启为止，中间每一次重新检查都清除不了它。现在只要应用变成最新状态，它就会立刻消失。至于那些属于仍在等待中的更新的失败原因，则不会被动它——这样你还没来得及看的原因，就不会被后台运行的检查悄悄清掉。
...
**DuoUpdater 现在会告诉你，让所有应用保持最新，实际在下载流量上花费了多少。** 它其实一直在精确到字节地统计为你获取的每一次更新——只是这个统计数字此前没地方显示，只能安静地躺在一个没人会去读的文件里。现在有了一个"下载流量"窗口，可以从菜单底部的图表按钮打开，本月的流量数字就直接印在那个按钮旁边，不用打开任何东西，最常见的那个问题就已经有答案了。窗口里有：总流量、最近三个月并排对比及其变化幅度、字节来源的构成分析，以及按花费多少给每个应用排名——点开某个应用，还能看到它经历过的每一次更新、版本从哪升到哪，以及那次下载的大小。
